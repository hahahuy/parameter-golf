...
def ttt_adapt_adamw(h,device,val_data,base_model):
	seq_len=h.eval_seq_len;total_seqs=(val_data.val_tokens.numel()-1)//seq_len;batch_seqs=h.ttt_batch_seqs
	if h.ttt_freeze_blocks>0:
		for i,block in enumerate(base_model.blocks):
			if i<h.ttt_freeze_blocks:
				for p in block.parameters():p.requires_grad_(False)
	ttt_params=[p for p in base_model.parameters() if p.requires_grad]
	log(f"ttt_adamw:trainable={sum(p.numel() for p in ttt_params)} frozen={sum(p.numel() for p in base_model.parameters() if not p.requires_grad)}")
	optimizer=torch.optim.AdamW(ttt_params,lr=h.ttt_lr,weight_decay=0.)
	scheduler=torch.optim.lr_scheduler.CosineAnnealingLR(optimizer,T_max=h.ttt_epochs,eta_min=h.ttt_lr*.1)
	my_start=(total_seqs*h.rank)//h.world_size;my_end=(total_seqs*(h.rank+1))//h.world_size
	base_model.train();t0=time.perf_counter()
	for epoch in range(h.ttt_epochs):
		epoch_loss_sum=torch.zeros((),device=device,dtype=torch.float64);epoch_tokens=torch.zeros((),device=device,dtype=torch.float64)
		for bs in range(my_start,my_end,batch_seqs):
			be=min(bs+batch_seqs,my_end);raw_start=bs*seq_len;raw_end=be*seq_len+1
			if raw_end>val_data.val_tokens.numel():continue
			local=val_data.val_tokens[raw_start:raw_end].to(device=device,dtype=torch.int64);x=local[:-1].reshape(-1,seq_len);y=local[1:].reshape(-1,seq_len)
			...
			if h.world_size>1:
				for p in ttt_params:
					if p.grad is not None:dist.all_reduce(p.grad,op=dist.ReduceOp.AVG)
			torch.nn.utils.clip_grad_norm_(ttt_params,1.);optimizer.step()
			epoch_loss_sum+=loss.detach().to(torch.float64)*float(y.numel());epoch_tokens+=float(y.numel())
		if h.world_size>1:dist.all_reduce(epoch_loss_sum,op=dist.ReduceOp.SUM);dist.all_reduce(epoch_tokens,op=dist.ReduceOp.SUM)
		scheduler.step()
		log(f"ttt_adamw:epoch {epoch+1}/{h.ttt_epochs} loss:{epoch_loss_sum.item()/max(epoch_tokens.item(),1):.4f} time:{time.perf_counter()-t0:.1f}s")
	for p in base_model.parameters():p.requires_grad_(True)
	base_model.eval();log(f"ttt_adamw:done elapsed={time.perf_counter()-t0:.1f}s")
...
def train_and_eval(h,device):
	random.seed(h.seed);np.random.seed(h.seed);torch.manual_seed(h.seed);torch.cuda.manual_seed_all(h.seed);val_data=ValidationData(h,device);log(f"train_shards: {len(list(Path(h.datasets_dir).resolve().glob('fineweb_train_*.bin')))}");log(f"val_tokens: {val_data.val_tokens.numel()-1}");base_model,compiled_model=train_model(h,device,val_data);torch._dynamo.reset();timed_eval('pre-quantization post-ema',eval_val,h,device,val_data,compiled_model)
	if h.ttt_enabled:
		del compiled_model;torch._dynamo.reset();torch.cuda.empty_cache();log(f"ttt_adamw:start lr={h.ttt_lr} epochs={h.ttt_epochs} freeze_blocks={h.ttt_freeze_blocks}");ttt_adapt_adamw(h,device,val_data,base_model);timed_eval('pre-quantization post-ttt',eval_val,h,device,val_data,torch.compile(base_model,dynamic=False,fullgraph=True))
